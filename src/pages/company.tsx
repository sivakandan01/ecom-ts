...
const CompanyModule = () => {
  ...
  const HandleSubmit = async (data: SelectedProp) => {
    try {
      ...
      toast(<Toast body={`Product ${text} successFully.`} color="bg-green-200" />, {
        unstyled: true,
      });
      FetchData();
    } catch (err: unknown) {
  ...
  const HandleDelete = async () => {
    try {
      ...
      toast(<Toast body="Product Deleted SuccessFully" color="bg-green-200" />, { unstyled: true });
    } catch (err: unknown) {
      if (typeof err === "object") {
        toast(<Toast body={err.response?.data.message}  color="bg-red-200" />, { unstyled: true });
      }
    }
    setOpenDialog(false);
    FetchData();
